这是一个为您准备好的 `DATABASE_DESIGN.md` 文件内容。您可以直接复制并保存为 Markdown 文件，提交给您的老师。

这份文档使用了标准的学术格式，包含了 **Mermaid 语法的 E-R 图**（在支持 Markdown 的编辑器如 Typora, VS Code, GitHub 中可以直接渲染为图片）以及详细的 **三范式分析报告**。
...
erDiagram
    ...
    APPOINTMENTS {
        string id PK "挂号单号"
        string patient_id FK "患者"
        string department_id FK "挂号科室"
        string doctor_id FK "接诊医生"
        string status "状态"
        datetime create_time "挂号时间"
    }

    MEDICAL_RECORDS {
        string id PK "病历编号"
        string patient_id FK "患者"
        string doctor_id FK "医生"
        text diagnosis "诊断结果"
    ...
    DEPARTMENTS ||--|{ APPOINTMENTS : "receives (接收)"
    
    DOCTORS ||--|{ APPOINTMENTS : "attends (接诊)"
    DOCTORS ||--|{ MEDICAL_RECORDS : "writes (开具)"
