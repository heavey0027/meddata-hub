...
export const findPatientByQuery = async (query: string | undefined | null): Promise<Patient | undefined> => {
  ...
  // Always fetch fresh list from backend (since we removed cache)
  const patients = await getPatients();
  
  // Try ID exact match first (Case insensitive)
  let found = patients.find(p => p.id.toLowerCase() === q.toLowerCase());
  if (found) return found;

  // Then try phone exact match
  found = patients.find(p => p.phone === q);
  if (found) return found;

  // Name match is risky but included for completeness in Mock
  return patients.find(p => p.name === q);
};
...
export const getExistingPatient = async (appointment: Appointment): Promise<Patient> => {
    ...
    if (appointment.patientId) {
        const patients = await getPatients(); // Fresh fetch
        const existing = patients.find(p => p.id === appointment.patientId);
        if (existing) {
