...
*   **无状态认证**: 通过在全局钩子中实现 **JWT** 认证机制，配合白名单功能，跳过不需要认证的接口（如登录接口等），实现接口级的权限校验。
